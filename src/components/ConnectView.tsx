...
import { activateVoucher, activateCoinSession } from '../services/wifiService';
...
const ConnectView: React.FC<ConnectViewProps> = ({ onConnect }) => {
    ...
    const handleCoinConnect = async () => {
        setIsLoading(true);
        setError('');
        try {
            const session = await activateCoinSession();
            onConnect(session);
        } catch (e: any) {
            setError(e.message || 'An unknown error occurred.');
        } finally {
    ...
    return (
        <div className="animate-fade-in">
            ...
            <form onSubmit={handleSubmit}>
                ...
                >
                    {isLoading ? 'Connecting...' : 'Connect with Voucher'}
                </button>
            </form>
            ...
            <div className="mt-6 text-center">
                ...
                >
                    {isLoading ? 'Connecting...' : 'Start 15 Minute Session (Coin)'}
                </button>
                <p className="text-xs text-gray-500 mt-2">Activates a timed session, just like inserting a coin.</p>
            </div>
        </div>
    );
};
