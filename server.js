...
// --- System configuration paths (defined early for broad access) ---
...
adminRouter.put('/network-config', adminAuth, (req, res) => {
    ...
    (async () => {
        try {
            console.log(`[Admin] Applying persistent network changes...`);
            
            // 1. Write the persistent network configuration for the hotspot interface.
            // This ensures the settings survive a reboot.
            const HOTSPOT_INTERFACE_CONF_PATH = isProd 
                ? '/etc/network/interfaces.d/60-sulit-wifi-hotspot' 
                : path.join(__dirname, '60-sulit-wifi-hotspot.mock');
            
            const interfaceConfig = [
                '# SULIT WIFI - Hotspot Interface Configuration',
                '# This file is auto-generated by the SULIT WIFI admin panel.',
            ...
            const tempConfName = `60-sulit-wifi-temp-${crypto.randomBytes(4).toString('hex')}`;
            const tempConfPath = path.join(os.tmpdir(), tempConfName);
            await fs.promises.writeFile(tempConfPath, interfaceConfig, 'utf-8');
            const moveCommand = isProd ? `sudo mv ${tempConfPath} ${HOTSPOT_INTERFACE_CONF_PATH}` : `mv ${tempConfPath} ${HOTSPOT_INTERFACE_CONF_PATH}`;
            await promiseExec(moveCommand);
            console.log(`[Admin] Wrote persistent network config to ${HOTSPOT_INTERFACE_CONF_PATH}`);

            // 2. Apply the new network configuration live using ifdown/ifup
            if (isProd) {
                console.log(`[Admin] Restarting interface ${hotspotInterface} to apply new static IP...`);
                // ifdown may fail if the interface is not configured, which is fine.
                await promiseExec(`sudo ifdown ${hotspotInterface}`).catch(e => console.warn(`ifdown ${hotspotInterface} failed (this is often ok):`, e.message));
                await promiseExec(`sudo ifup ${hotspotInterface}`);
                console.log(`[Admin] Interface ${hotspotInterface} is up with new configuration.`);
            } else {
                 console.log('[Admin] Mock mode: Skipped ifdown/ifup.');
            }
            ...
            if (hotspotDhcpServer.enabled) {
                ...
                if (isProd) {
                    const dnsmasqDir = path.dirname(DNSMASQ_HOTSPOT_CONF_PATH);
                    await fs.promises.mkdir(dnsmasqDir, { recursive: true });
                }
                const dnsmasqConfig = [
                    '# SULIT WIFI - Hotspot DHCP Configuration',
                    'bogus-priv', 'domain-needed',
                    'except-interface=lo',
                    ...
                    `dhcp-option=interface:${hotspotInterface},6,${hotspotIpAddress}`,
                ].join('\n');
            ...
            // 4. Update splash.html redirect and nodogsplash config atomically
            console.log('[Admin] Updating splash.html to ensure correct redirect IP.');
            const defaultSplashContent = getDefaultSplashContent();
            const tempSplashName = `splash-temp-update-${crypto.randomBytes(4).toString('hex')}.html`;
            const tempSplashPath = path.join(os.tmpdir(), tempSplashName);

            try {
                await fs.promises.writeFile(tempSplashPath, defaultSplashContent, 'utf-8');
                const moveSplashCommand = isProd ? `sudo mv ${tempSplashPath} ${SPLASH_HTML_PATH}` : `mv ${tempSplashPath} ${SPLASH_HTML_PATH}`;
                await promiseExec(moveSplashCommand);
                console.log(`[Admin] Successfully updated splash.html with new IP: ${hotspotIpAddress}`);
            } catch (e) {
                console.error(`[Admin] FAILED to update splash.html during network config change:`, e.stderr || e.message);
            } finally {
                if (fs.existsSync(tempSplashPath)) fs.unlinkSync(tempSplashPath);
            }
...
if (!isProd && !fs.existsSync(SPLASH_HTML_PATH)) {
    fs.writeFileSync(SPLASH_HTML_PATH, getDefaultSplashContent());
}
...
adminRouter.get('/portal-html', adminAuth, async (req, res) => {
    try {
        ...
        console.error(`[Admin] Error reading portal HTML from ${SPLASH_HTML_PATH}:`, error);
...
adminRouter.put('/portal-html', adminAuth, async (req, res) => {
    ...
    try {
        ...
        const command = isProd ? `sudo mv ${tempFilePath} ${SPLASH_HTML_PATH}` : `mv ${tempFilePath} ${SPLASH_HTML_PATH}`;
        await promiseExec(command);

        console.log(`[Admin] Successfully updated ${SPLASH_HTML_PATH}`);
        res.status(204).send();
    } catch (error) {
...
adminRouter.post('/portal-html/reset', adminAuth, async (req, res) => {
    ...
    try {
        ...
        console.log(`[Admin] Successfully reset ${SPLASH_HTML_PATH} to default.`);
