        ...
        try {
            ...
            if (hotspotDhcpServer.enabled) {
                ...
                const dnsmasqConfig = [
                    ...
                    '',
                    '# --- General Settings ---',
                    '# Prevent DNS forwarding of queries for private IP ranges and unqualified names.',
                    'bogus-priv',
                    'domain-needed',
                    '',
                    '# --- Interface Settings ---',
                    '# This configuration ensures dnsmasq only serves DHCP on the specified hotspot interface.',
                    '# Explicitly ignore the loopback interface to prevent system resolver conflicts.',
                    'except-interface=lo',
                    '# Only bind to the specified hotspot interface, not all of them.',
                    `interface=${hotspotInterface}`,
                    'bind-interfaces',
                    '',
                    '# Only listen for DNS queries on the hotspot interface IP.',
                    `listen-address=${hotspotIpAddress}`,
                    '',
                    '# --- DNS Hijacking for Captive Portal ---',
                    '# Resolve all DNS queries to the portal IP for unauthenticated clients.',
                    `address=/#/${hotspotIpAddress}`,
                    '',
                    '# --- DHCP Settings ---',
                    '# Tell dnsmasq it is the authoritative DHCP server on this subnet to avoid conflicts.',
                    'dhcp-authoritative',
                    '',
